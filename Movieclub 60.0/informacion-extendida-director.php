...
<div class="style8" id="content">
  <div align="center">
    <p>
      <?php
  	//Cogemos la id del director que se desea ver
	$iddirector=$_GET["id"];
	//Hacemos 2 consultas
	//PRIMERA CONSULTA: Consultamos la información del director 
	$verdirector=conexion("SELECT * FROM directores WHERE directores.id=$iddirector");
	//SEGUNDA CONSULTA: Consultamos las películas que ha dirigido
	$peliculasdirigido= conexion("SELECT peliculas.id, titulo, cartel 
	FROM peliculas INNER JOIN directores ON directores.id=peliculas.id_director WHERE id_director=$iddirector");
	$numeropeliculasdirigido=mysql_num_rows($peliculasdirigido); 		//Contamos las filas de la segunda consulta
  ?>
    </p>
  </div>
  <table width="100%" border="2" bordercolor="#0033FF" bgcolor="#CCCCCC">
	...
	//Creamos la tabla principal con la información de la primera consulta
		while($filasdirectorelegido=mysql_fetch_array($verdirector)){
		echo "<span class=\"style9\"><b>".$filasdirectorelegido["nombre"]."</span></b>";
		echo"<p>&nbsp;</p>";
		echo"<p>&nbsp;</p>";
		echo "<div align=\"center\"><img src=\"".$filasdirectorelegido["foto_director"]."\" width=\"500\" height=\"500\" />";
		echo"<p>&nbsp;</p>";
		echo "<div align=\"left\"><b class=\"style18\">Biografía: </b>".$filasdirectorelegido["biografia_director"]."";
		}
		echo"<p>&nbsp;</p>";
		//Comprobamos las películas que ha dirigido
		//Si no tiene ninguna película
		if($numeropeliculasdirigido==0)
		{
			//Mostraremos un mensaje de que no ha dirigido ninguna película de las que tenemos almacenadas en la BD 
			echo " <p align=\"center\" class=\"style13\">
			No ha dirigido ninguna película</p>";
		}
		else
		{
			//Si tiene una o más películas dirigidas mostramos un mensaje de cuantas películas ha dirigido
			echo " <p align=\"center\" class=\"style13\">Ha dirigido $numeropeliculasdirigido películas</p>";
			while($filasdirigido=mysql_fetch_array($peliculasdirigido)){
			//Mostraremos el cartel y el título de la película con su enlace a la información extendida
					echo "<div align=\"center\"><img src=\"".$filasdirigido["cartel"]."\" width=\"200\" height=\"200\" />";
					echo "<div align=\"center\"><a href=\"informacion-extendida.php?id=".$filasdirigido["id"]."\">
					".$filasdirigido["titulo"]."</a>";
					echo"<p>&nbsp;</p>";
				}
